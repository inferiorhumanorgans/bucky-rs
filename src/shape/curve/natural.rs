...
impl CurveContext for CurveNaturalContext {
    ...
    fn end_line(&mut self) {
        ...
        if n > 0 {
            self.path.move_to(Vector2F::new(self.x[0] as f32, self.y[0] as f32));
            if n == 2 {
                self.path.line_to(Vector2F::new(self.x[1] as f32, self.y[1] as f32));
            } else if n > 1 {
...
fn expected_results() {
    ...
    // NOTE: Rust string formatting rounds with more precision than Javascript by default
    {
        ...
        assert_eq!("M 0 1 C 0.33333334 2 0.6666667 3 1 3 C 1.3333334 3 1.6666666 2 2 1", line.generate(data));
    }
